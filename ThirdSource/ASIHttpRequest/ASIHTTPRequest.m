...
- (void)startRequest
{
    ...
    if([[[[self url] scheme] lowercaseString] isEqualToString:@"https"]) {       
        ...
        if (![self validatesSecureCertificate]) {
            ...
            /*
             1.如果请求打头是HTTPS://,则需要设置SSL,ASI SSL如果失效就设置sslProperties的kCFStreamSSLValidatesCertificateChain字段为false，
             2.如果被设置为使用用户证书，将用户设置的证书Array以kCFStreamSSLCertificates作为key加入sslProperties
             3.使用CFReadStreamSetProperty设置readStream的ssl读取信息
             */
...
+ (void)registerForNetworkReachabilityNotifications
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:YIZHIReachabilityChangedNotification object:nil];
}


+ (void)unsubscribeFromNetworkReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:YIZHIReachabilityChangedNotification object:nil];
}
