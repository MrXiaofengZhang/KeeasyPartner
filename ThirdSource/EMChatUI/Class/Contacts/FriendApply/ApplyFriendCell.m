...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _addButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 60, 0, 50, 25)];
        _addButton.layer.cornerRadius = mygap;
        [_addButton setBackgroundColor:SystemBlue];
        ...
        _refuseButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 120, 0, 50, 25)];
        _refuseButton.layer.cornerRadius = mygap;
        [_refuseButton setBackgroundColor:color_red_dan];
...
-(void)layoutSubviews
{
    ...
    _addButton.frame = CGRectMake(self.contentView.frame.size.width - 60, (self.contentView.frame.size.height - 30) / 2, 50, 25);
    _refuseButton.frame = CGRectMake(self.contentView.frame.size.width - 120, (self.contentView.frame.size.height - 30) / 2, 50, 25);
...
+ (CGFloat)heightWithContent:(NSString *)content
{
    if (!content || content.length == 0) {
        ...
        CGSize size = [content boundingRectWithSize:CGSizeMake(BOUNDS.size.width - 60 - 120, CGFLOAT_MAX)  options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14.0]} context:nil].size;
