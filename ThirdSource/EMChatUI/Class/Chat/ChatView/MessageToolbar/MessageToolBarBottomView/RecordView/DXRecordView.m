...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _textLabel.text = NSLocalizedString(@"手指上滑,取消发送", @"松开手指,取消发送");
        [self addSubview:_textLabel];
        _textLabel.font = [UIFont systemFontOfSize:13];
        _textLabel.textColor = [UIColor whiteColor];
        _textLabel.layer.cornerRadius = 3;
        _textLabel.layer.borderColor = [NavgationColor colorWithAlphaComponent:0.5].CGColor;
...
{
    _textLabel.text = NSLocalizedString(@"松开手指,取消发送", @"松开手指,取消发送");
    _textLabel.font = [UIFont boldSystemFontOfSize:13];
    _textLabel.backgroundColor = NavgationColor;
}
