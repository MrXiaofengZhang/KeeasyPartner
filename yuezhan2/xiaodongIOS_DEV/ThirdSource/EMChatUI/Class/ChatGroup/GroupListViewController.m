...
//    [createButton setImage:[UIImage imageNamed:@"group_chat_wpc"] forState:UIControlStateNormal];
...
//    UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithCustomView:backButton];
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"back"] style:UIBarButtonItemStylePlain target:self action:@selector(popViewControllerAnimated:)];
    [[EaseMob sharedInstance].chatManager asyncFetchMyGroupsListWithCompletion:^(NSArray *groups, EMError *error) {
        ...
        if (self.dataSource.count==0) {
            self.tableView.backgroundView = [[EmpatyView alloc]initWithImg:@"emptyChat" AndText:@"您还没有群聊信息"];
        }
        else{
    ...
    //注册消息个数监听这个可以跟新消息个数
    //[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshList:) name:NotificationRefreshMessageCount object:nil];
}
...
- (EMSearchDisplayController *)searchController
{
    if (_searchController == nil) {
        ...
        [_searchController setDidSelectRowAtIndexPathCompletion:^(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            EMGroup *group = [weakSelf.searchController.resultsSource objectAtIndex:indexPath.row];
            ChatViewController *chatVC = [[ChatViewController alloc] initWithChatter:group.groupId isGroup:YES WithIcon:@""];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.unreadCount = [[EaseMob sharedInstance].chatManager unreadMessagesCountForConversation:[LVTools mToString: group.groupId]];
    NSString *iconPath =[LVTools mToString: [kUserDefault objectForKey:group.groupId]];
    [cell.imageView sd_setImageWithURL:[NSURL URLWithString: [NSString stringWithFormat:@"%@%@",preUrl,iconPath]] placeholderImage:[UIImage imageNamed:@"groupPrivateHeader"]];
        NSLog(@"%d",(int)(cell.unreadCount));
    return cell;
}
