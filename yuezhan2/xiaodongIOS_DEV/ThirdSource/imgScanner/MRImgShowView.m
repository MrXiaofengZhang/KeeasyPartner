...
- (id)initWithFrame:(CGRect)frame withSourceData:(NSMutableArray *)imgSource withIndex:(NSInteger)index andvc:(UIViewController *)vc hasUseUrl:(BOOL)useUrl{
    
    self = [super initWithFrame:frame];
    if (self) {
        self.superVC = vc;
        self.useUrl = useUrl;
        self.arr = [[[NSArray alloc] initWithArray:imgSource] autorelease];
        // 初始化控件属性
        [self initScrollView];
...
{
    if (!_page) {
        _page = [[UIPageControl alloc] initWithFrame:CGRectMake(0, 0, 30*[self.arr count], 20)];
        _page.center = CGPointMake(UISCREENWIDTH/2, UISCREENHEIGHT-100);
        _page.numberOfPages = [self.arr count];
        _page.currentPage = _curIndex;
        [self.superVC.view addSubview:_page];
        [_page release];
    }
}
...
- (UIImageView *)creatImageView{
    ...
    UIImageView *imgView = [[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, width, height)] autorelease];
...
- (UIScrollView *)scrollViewWithPosition:(MRImgLocation)imgLocation withImgView:(UIImageView *)imgView{
    ...
    UIScrollView *scrollView = [[[UIScrollView alloc] initWithFrame:CGRectMake(width * imgLocation, 0, width, height)] autorelease];
...
//        正常
    ...
    if (sCount) {
        ...
        // 首位
        if (curIndex == 0) {
            urlL = [[[NSURL alloc] initWithString:[_imgSource lastObject]] autorelease];
            urlC = [[[NSURL alloc] initWithString:_imgSource[curIndex]] autorelease];
            long nextIndex = curIndex == sCount - 1 ? curIndex : curIndex + 1;
            urlR = [[[NSURL alloc] initWithString:_imgSource[nextIndex]] autorelease];
        // 末位
        } else if (curIndex == sCount - 1){
            long lastIndex = curIndex == 0 ? curIndex : curIndex - 1;
            urlL = [[[NSURL alloc] initWithString:_imgSource[lastIndex] ] autorelease];
            urlC = [[[NSURL alloc] initWithString:[_imgSource lastObject]] autorelease];
            urlR = [[[NSURL alloc] initWithString:_imgSource[0]] autorelease];
        // 中间
        } else {
            urlL = [[[NSURL alloc] initWithString:_imgSource[curIndex - 1] ] autorelease];
            urlC = [[[NSURL alloc] initWithString:_imgSource[curIndex]] autorelease];
            urlR = [[[NSURL alloc] initWithString:_imgSource[curIndex + 1]]autorelease];
        }
        
        imgList = [[[NSArray alloc] initWithObjects:urlL, urlC, urlR, nil] autorelease];
    }
...
- (NSArray *)imgListFromIndex:(NSInteger)curIndex{
    ...
    if (sCount) {
        ...
        imgList = [[[NSArray alloc] initWithObjects:imgL, imgC, imgR, nil] autorelease];
    }
...
- (void)setAllImgVContentFromImage:(NSArray *)imgList{
    ...
    if (imgList.count) {
        if (self.useUrl == YES) {
            [vLift sd_setImageWithURL:imgList[MRImgLocationLeft] placeholderImage:nil];
            [vCenter sd_setImageWithURL:imgList[MRImgLocationCenter] placeholderImage:nil];
            [vRight sd_setImageWithURL:imgList[MRImgLocationRight] placeholderImage:nil];
        }else{
