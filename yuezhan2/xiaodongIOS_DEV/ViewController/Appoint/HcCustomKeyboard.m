...
{
    if (self.wpcBackView) {
        [self.wpcBackView removeFromSuperview];
        self.wpcBackView = nil;
    }
    if (self.mBackView) {
...
-(void)textViewShowView:(UIViewController *)viewController customKeyboardDelegate:(id)delegate andState:(BOOL)state
{
    self.mViewController =viewController;
    self.mDelegate =delegate;
    self.isTop = NO;//初始化的时候设为NO
    self.appearState = state;
    self.wpcBackView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, WIDTH_SCREEN, HEIGHT_SCREEN)];
    self.wpcBackView.backgroundColor = [UIColor blackColor];
    self.wpcBackView.alpha = 0.4;
    [self.mViewController.view addSubview:self.wpcBackView];
    
    UITapGestureRecognizer *tapTheGrand = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(disappear)];
    [self.wpcBackView addGestureRecognizer:tapTheGrand];
    ...
    self.mSecondaryBackView =[[UIView alloc]initWithFrame:CGRectMake(10, 10, UISCREENWIDTH-80, 30)];
    self.mSecondaryBackView.backgroundColor = [UIColor clearColor];
    [self.mBackView addSubview:self.mSecondaryBackView];
    
    self.mTextView =[[PlaceholderTextView alloc]initWithFrame:CGRectMake(1, 1, UISCREENWIDTH-90, 28)];
    ...
    self.mTalkBtn.frame =CGRectMake(self.mSecondaryBackView.right+10, 16, 30, 18);
...
- (void)keyboardFrameChange:(NSNotification*)notification
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.mBackView.frame =CGRectMake(0, HEIGHT_SCREEN-_keyboardRect.size.height-94, WIDTH_SCREEN, 50);
...
{    
    self.isTop =NO;
        [UIView animateWithDuration:0.3 animations:^{
            if (self.appearState == YES) {
                self.mBackView.frame=CGRectMake(0, HEIGHT_SCREEN, WIDTH_SCREEN, 50);
            } else {
                self.mBackView.frame=CGRectMake(0, HEIGHT_SCREEN-50, WIDTH_SCREEN, 50);
            }
        
        self.mHiddeView.alpha =0.0f;
        self.mSecondaryBackView.frame =CGRectMake(10, 10, UISCREENWIDTH-80,30);
    } completion:^(BOOL finished) {
...
- (void)textDidChanged:(NSNotification *)notif //监听文字改变 换行时要更改输入框的位置
{
    ...
    CGFloat selfHeight = self.mTextView.superview.frame.origin.y * 2 + contentSize.height + 2 * 5;
    ...
    self.mSecondaryBackView.frame =CGRectMake(10, 10, UISCREENWIDTH-80, selfHeight-20);
    NSLog(@"%f\n%f\n%f",selfHeight,contentSize.height,self.mTextView.superview.frame.origin.y);
}
