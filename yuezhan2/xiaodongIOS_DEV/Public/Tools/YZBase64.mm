...
+(NSString *)encode:(NSData *)data
{
    if (data.length == 0)
        return nil;
     char *characters = (char*)malloc(data.length * 3 / 2);
    ...
    while (index <= end) {
        ...
        characters[charCount++] = encodingBaseTable[(d >> 18) & 63];
        characters[charCount++] = encodingBaseTable[(d >> 12) & 63];
        characters[charCount++] = encodingBaseTable[(d >> 6) & 63];
        characters[charCount++] = encodingBaseTable[d & 63];
